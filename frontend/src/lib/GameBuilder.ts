...
export class GameBuilder {
  ...
  playerGuesses(
    ...
    const guessResultIndex = findIndex((guessResult) => {
    ...
    if (guessResultIndex >= 0) {
      ...
      if (
        this.game.guessResults[guessResultIndex].guesses.length >= GUESS_LIMIT
      ) {
  ...
  playerPicks(
    ...
    if (isGameComplete(this.game)) {
      return GameBuilder.fromChallenge(
        challenge,
        otherPlayer(player, this.game),
      );
    }
  ...
  playerDraws(
    player: StreakGuessingGamePlayer,
    drawing: StreakGuessingGameDrawing,
  ) {
