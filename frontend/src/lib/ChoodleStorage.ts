...
export async function createUncommittedChoodle(
  undoStack: UndoStack,
  ...
  const upScaledUploadResult = (async () => {
    ...
    const upScaledImageBlob = await (await fetch(upScaledImage as unknown as URL)).blob();
  ...
  return {
    transaction: readWriteClient
      ...
      .create(cmsChoodle)
      .patch(creatorId, (p) =>
        p.setIfMissing({ choodles: [] }).append('choodles', [{ _ref: choodleId }])
      ),
    choodleId: choodleId,
  };
}

export async function saveChoodle(
  undoStack: UndoStack,
  canvas: HTMLCanvasElement,
  extraMetadata?: any,
  creatorId?: string
