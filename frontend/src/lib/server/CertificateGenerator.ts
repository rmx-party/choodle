...
export const generateCertificateFor = async ({
  choodleId,
  creatorEmail,
}: {
  choodleId: string;
  ...
  const font = await Jimp.loadFont(
    `${PUBLIC_URL_BASE}/open-sans/open-sans-64-black/open-sans-64-black.fnt`
  );
  const fontHeader = await Jimp.loadFont(
    `${PUBLIC_URL_BASE}/open-sans/open-sans-128-black/open-sans-128-black.fnt`
  );
  const fontTiny = await Jimp.loadFont(
    `${PUBLIC_URL_BASE}/open-sans/open-sans-16-black/open-sans-16-black.fnt`
  );
  ...
  const qrcode = await QRCode.toDataURL(`${PUBLIC_URL_BASE}/c/${choodleId}`, {
    errorCorrectionLevel: 'L',
    scale: 8,
    margin: 0,
  });
  const qrcodeImage = (await Jimp.read(Buffer.from(qrcode.split(',')[1], 'base64'))).resize(
    256,
    256
  );
