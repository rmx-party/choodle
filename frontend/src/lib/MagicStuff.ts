...
async function mint(
  ...
  //         console.error(err)
  //     });
  return await contract.safeMint(address, data).catch((err) => {
    // TODO: run a reboot callback to reload canvas etc
    loading.set(false);
    console.error(err);
  });
...
export async function connectAndMint(imageData: string, creatorId: string, createdAt: string) {
  ...
  loading.set(true);
  const { provider, accounts, magic } = await connectMagic().catch((err) => {
    // TODO: run a reboot callback to reload canvas etc
    loading.set(false);
    console.error(err);
  ...
  const receipt = await preReceipt.wait().catch((error) => {
    // TODO: run a reboot callback to reload canvas etc
    loading.set(false);
    console.error(error);
  });
  ...
  const tokenId = fp.first(
    fp.compact(
      fp.map((bigNumber) => bigNumber?.toNumber())(
        fp.map((event) => event.args?.tokenId)(receipt.events)
      )
    )
  );
  return tokenId;
...
export async function sendAChoodle(tokenId) {
  ...
  const preReceipt = await contract.safeTransferFrom(address, to, tokenId).catch((err) => {
    // TODO: run a reboot callback to reload canvas etc
    loading.set(false);
    console.error(err);
  });
