...
export const POST = async ({ request, cookies }) => {
  ...
  const certificateUploadResult = await readWriteClient.assets.upload('image', supposedBuffer, {
    timeout: 5000,
  });

  const newChoodle = await readWriteClient
    .patch(choodle._id)
    .set({
      certificate: {
        _type: 'image',
...
<br />
<img width="300" src=${urlFor(newChoodle.certificate)
    .width(300)
    .url()} alt='Choodle Certificate of Ownership' />
<br />
