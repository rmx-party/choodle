<script lang="ts">
    ...
    const mint = (choodleId: string) => {
        return async () => {
            ...
            if (tokenId) {
                await readWriteClient.patch(choodle._id).set({shouldMint: false, mintedAt: new Date().toISOString(), tokenId: tokenId}).commit()
            }
        }
    }
    ...
    const filterChoodles = (choodles: any[], filterState: string) => {
        ...
        const activeFilter = {
            ...
            'should-not-mint': fp.filter(c => !c.shouldMint && !c.tokenId &&
                !c.sentAt),
