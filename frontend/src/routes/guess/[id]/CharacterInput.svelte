<script lang="ts">
  ...
  currentGuess.subscribe((newCurrentGuess) => {
    const value = newCurrentGuess.length === singleSpacedFormat.length ? true : false;
  ...
  const onInput = (event: Event) => {
    currentGuess.set(
      replaceCharAt($currentGuess, Number(event.target.dataset.index), event.target.value)
    );
